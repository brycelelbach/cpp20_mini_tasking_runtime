...
// TODO: use barrier moar (replace latch in startup code with barrier).
...
int main()
{
...

  {
    ...
    std::vector<unique_future<std::uint64_t, decltype(tm.get_executor())>> fs;

    for (std::size_t i = 0; i < 64; ++i)
      fs.emplace_back(afibonacci(tm.get_executor(), 10));

    results = when_all(tm.get_executor(), std::move(fs)).get();
  }

  for (std::uint64_t r : results)
    std::cout << "afibonacci(10) == " << r << "\n";
}
