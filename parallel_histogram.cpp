...
template <typename T>
std::atomic<T>& atomic_ref(T& t) {
  return *reinterpret_cast<std::atomic<T>*>(&t);
}

} // namespace std

template <typename I, typename O, typename T, typename OP>
void histogram(I first, I last, O output, T increment, OP op)
{
  auto const elements = std::distance(first, last);
  for (std::uint64_t i = 0; i < elements; ++i)
    output[op(first[i])] += increment;
}

template <typename EP, typename I, typename O, typename T, typename OP>
void histogram(EP&& exec, I first, I last, O output, T increment, OP op)
{
  std::uint64_t const elements = std::distance(first, last);
  std::uint64_t const chunks   = (1 + ((elements - 1) / exec.chunk_size())); // Round up.
  ...
  for (std::uint64_t chunk = 0; chunk < chunks; ++chunk)
    exec.submitter().submit(
      [=, &l, &exec] {
        auto const this_begin = chunk * exec.chunk_size();
        auto const this_end   = std::min(elements, (chunk + 1) * exec.chunk_size());

        for (std::uint64_t i = this_begin; i < this_end; ++i)
          std::atomic_ref(output[op(first[i])]).fetch_add(increment, std::memory_order_relaxed);

        l.count_down();

        while (!l.try_wait())
          exec.submitter().boost_block();
      }
    );
...
int main(int argc, char** argv) {
  ...
  std::uint64_t smallest_element = 0;
  std::uint64_t largest_element  = 4;

  std::vector<std::uint64_t> input(elements);

  std::vector<std::uint64_t> gold(largest_element - smallest_element);
  std::vector<std::uint64_t> parallel(largest_element - smallest_element);

  {
    std::mt19937 gen(1337);
    std::uniform_int_distribution<std::uint64_t> dis(smallest_element, largest_element);
    std::generate_n(input.begin(), elements, [&] { return dis(gen); });
  }
...

  {
    ...
    histogram(input.begin(), input.end(), gold.begin(), std::uint64_t(1), std::identity{});
...

  {
    ...
    auto const start = std::chrono::high_resolution_clock::now();
    histogram(execution_policy(tm, task_chunk_size),
              input.begin(), input.end(), parallel.begin(),
              std::uint64_t(1), std::identity{});
    auto const end   = std::chrono::high_resolution_clock::now();
  ...
  ALGOLOG("parallel complete");

  if (!std::equal(gold.begin(), gold.end(), parallel.begin()))
    std::cout << "PARALLEL HISTOGRAM RESULT FAILED COMPARISON WITH GOLD\n";

  if (512 >= elements)
    for (std::uint64_t i = 0; i < gold.size(); ++i)
      std::cout << "gold[" << i << "](" << gold[i]
                << ") parallel[" << i << "](" << parallel[i]
                << ")\n";

  std::cout << "time_gold(" << time_gold
            << " [sec]) time_parallel(" << time_parallel
            << " [sec])\n";
}
