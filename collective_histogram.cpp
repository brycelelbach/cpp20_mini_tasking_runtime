...
unique_future<unused> async_histogram(EP&& exec, I first, I last, O output, T increment, OP op)
{
  std::uint64_t const elements = std::distance(first, last);
  std::uint64_t const chunks   = (1 + ((elements - 1) / exec.chunk_size())); // Round up.
...
int main(int argc, char** argv) {
...

  {
    ...
    auto const start = std::chrono::high_resolution_clock::now();
    async_histogram(execution_policy(tm, task_chunk_size),
                    input.begin(), input.end(), parallel.begin(),
                    std::uint64_t(1), std::identity{}).get(tm);
    auto const end   = std::chrono::high_resolution_clock::now();
